// WIP
import { HTMLElement as ParserHTMLElement, Node as ParserNode, NodeType, parse } from "node-html-parser";
...
export const htmlToSlateTree = (htmlOrEl: NodeOrHtml, strict = false): SlateItem[] => {
  return getChildNodes(htmlOrEl)
    .map((el) => htmlNodeToSlateItem(el, strict))
    .filter((val) => !!val) as SlateItem[];
};
