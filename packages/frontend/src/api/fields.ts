...
import { MemberField, MemberFieldType, MemberFieldWithValue } from '@plurali/common/src/system'
...
const color = (string: string): string | null => {
  if (string && string.length >= 1) {
    string = xss(string)
    if (
      /^((#[a-fA-F0-9]{6})|(#[a-fA-F0-9]{8})|([a-fA-F0-9]{6})|([a-fA-F0-9]{8}))$/.test(string)
    ) {
      return string
    }
  }
...
const month = (string: string): string =>
  new Date(xss(string)).toLocaleString('default', { month: 'long' })
...
const monthDay = (string: string): string =>
  `${month(string)} ${new Date(xss(string)).getDay()}`

export const fieldConvertors: Record<
  MemberFieldType,
  (val: string, md: boolean) => string | null
> = {
...
export function hasValue(value: MemberField|MemberFieldWithValue): value is MemberFieldWithValue {
