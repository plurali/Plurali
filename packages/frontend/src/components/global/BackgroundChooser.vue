<template>
  ...
  >
    <template #icon>
      <Component
        :is="backgroundType === Background.Image ? 'PhotoIcon' : 'SwatchIcon'"
    ...
    <form @submit.prevent="onSubmit" :enctype="backgroundType !== Background.Image ? 'multipart/form-data' : ''">
      <Uploader v-if="backgroundType === Background.Image" v-model="backgroundImage" />
...
import { SystemData, SystemMemberData, updateMember, updateSystem } from '../../api/system';

import { isHex } from '@plurali/common/src/utils/colors';
...
import { updateMemberBackgroundImage } from '../../api/system';
import { updateSystemBackgroundImage } from '../../api/system';

export default defineComponent({
  props: {
    entity: {
      type: Object as PropType<WithBackground & { id: string }>,
      required: true,
    },
    type: {
  ...
  setup(props, { emit }) {
    ...
    const backgroundType = ref<Background>(props.entity.data.backgroundType);
    ...
    const hasProperData = computed(() => {
      if (backgroundType.value === Background.Image) return !!backgroundImage.value;
      else return !!backgroundColor.value && backgroundColor.value !== initialColor;
    });

    const setType = (type: Background) => (backgroundType.value = type);
    ...
    const updateColor = async (color: string | null) => {
      ...
      const res = await wrapRequest<SystemData | SystemMemberData>(() =>
    ...
    const onSubmit = async () => {
      ...
      if (backgroundType.value === Background.Image && !!backgroundImage.value) {
    ...
    return {
      ...
      backgroundImage,
      backgroundColor,
      types: Object.keys(Background) as Background[],
      setType,
      updateColor,
      updateImage,
      open,
