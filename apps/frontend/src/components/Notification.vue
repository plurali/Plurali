<template>
  <div
    ...
    :class="['relative flex justify-between space-x-10 rounded-2xl bg-opacity-75 p-4', containerClass]"
  >
...
export default defineComponent({
  ...
  setup({ color }) {
    const containerStyle = computed(() =>
      color?.startsWith("#") && isHex(color) ? { backgroundColor: color, color: "#fff" } : {},
    );
    const containerClass = computed(() =>
      color && Object.keys(FlashType).includes(color) ? flashTypeToClass[color as FlashType] : "",
    );
