<template>
  <Fetchable :result="members" retry-text="Load members" :retry="fetchMembers">
    <div v-if="members" class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-5">
      <MemberSmallCard v-for="member of members" :key="member.id" :member="member" />
    </div>
    ...
    >
      ...
      <div class="flex flex-1 justify-between sm:justify-end gap-2">
        <button
          ...
          class="relative inline-flex items-center rounded-xl bg-white px-4 py-1.5 text-sm font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus-visible:outline-offset-0"
          @click="onPrevious"
        >
          Previous
        </button>
