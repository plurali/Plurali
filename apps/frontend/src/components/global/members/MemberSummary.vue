<template>
  <div v-if="data.member">
        ...
        <div>
          ...
          <PageTitle class="text-violet-700 inline-flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <span v-if="isDashboard" class="inline-flex flex-col sm:flex-row sm:items-center justify-center gap-3">
              ...
              />
              <a
                v-if="data.member.data.visible && data.system"
          ...
          <Subtitle v-if="data.member.description" class="mb-3">
            <Sanitized :value="string(data.member.description, true)" />
          </Subtitle>

          <div
            class="inline-flex flex-col md:flex-row justify-left md:justify-between md:items-center gap-2 text-gray-700"
          >
    ...
    <CustomFields
      ...
      :modifiable="isDashboard"
      :hide-no-values="!isDashboard"
      title="Custom Fields"
    />
    ...
    <Fetchable :result="data.pages" :retry="refetch">
      <PageFields
        ...
        :modifiable="isDashboard"
        owner-type="member"
        :member-id="data.member.data.slug!"
      />
    </Fetchable>
  </div>
</template>
