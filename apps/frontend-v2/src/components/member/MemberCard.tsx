  ...
  return (
    <Link
      ...
      className={c(
        ...
        onToggleVisibility
          ? member.data.visibility === Visibility.Public
            ? 'border-l-green-500'
            : 'border-l-red-500'
          : '',
        loading && 'bg-gray-200',
    ...
    >
      {member.avatar ? (
        <img src={member.avatar} alt={member.name} className="w-16 h-16 rounded-full object-cover" loading="lazy" />
      ) : (
      ...
      <div>
        <h2 className="text-xl font-medium">
          {member.name}
          {member.pronouns && <span className="text-sm text-gray-500 font-normaly">{member.pronouns}</span>}
        </h2>
        {member.description && (
