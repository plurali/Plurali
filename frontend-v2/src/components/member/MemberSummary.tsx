...
export const MemberSummary = ({ member, dashboardMode = false }: MemberSummaryProps) => {
  return (
    <div className="mb-5 flex flex-col text-center sm:flex-row sm:text-left justify-left items-center gap-4">
      {member.avatar && (
        <img src={member.avatar} alt={member.name} className="flex-shrink-0 w-32 h-32 rounded-full object-cover" />
      )}
      ...
      <div>
        ...
        <Title className="inline-flex flex-col sm:flex-row items-center justify-center gap-3">{member.name}</Title>
