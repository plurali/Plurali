...
export const Tag = ({ onClassName, offClassName, className, toggled, disabled = false, onToggle, children }: PropsWithChildren<TagProps>) => {
    const Element = onToggle ? "button" : "span";

    return <Element onClick={() => onToggle?.(!toggled)} className={c(
