...
export const fieldAndMemberSchema = { ...idSchema }.prop(
  'body',
  S.object().prop('visible', S.boolean()).prop('description', S.string())
)

export const systemSchema = S.object().prop(
  'body',
  S.object().prop('visible', S.boolean()).prop('description', S.string())
)
...
export default controller(async server => {
  ...
  server.post<SystemSchema>(
    '/',
    {
      schema: systemSchema.valueOf(),
    },
    async (req, res) =>
      withSystemContext(
        {
  ...
  server.post<FieldAndMemberSchema>(
    '/members/:id',
    {
      schema: fieldAndMemberSchema.valueOf(),
    },
    async (req, res) =>
      withSystemContext(
        {
        ...
        async ({ system, member: get }) => {
          ...
          res.send(
            data({
              member: await get(req.params.id),
            })
          )
        }
      )
    ...
    async (req, res) =>
      withSystemContext(
        ...
        async ({ system, member: get }) => {
          ...
          res.send(
            data({
              field: transformMemberField(field.pluralId, system, field),
            })
          )
        }
      )
