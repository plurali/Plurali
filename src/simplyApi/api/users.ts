...
export const transformMemberField = (id: string, system: UserEntry | System, data: UserField) => {
  const field =
    system instanceof System ? system.fields.find(f => f.fieldId === id) : system.content.fields[id]
  ...
  return new MemberField(id, field.name, position, type, visibility, UserFieldDataDto.from(data))
}
